...
app.post('/api/login', async function (req, res) {
  ...
  const chatroomUsers = await db.User.find({ $where: 
    function() { return (this.session && this.session.length>10 && this.chatroom && this.chatroom == chatroom) } }, 'name thumbnail');
...
app.get('/api/chatroom', async function (req, res) {
  ...
  io.sockets.emit('broadcast',`room:${gotoChatroom}`, { action: 'joined', 
              user: userRoomInfo(dbUser) } );
  io.sockets.emit('broadcast',`room:${chatroom}`, { action: 'left', 
              user: userRoomInfo(dbUser) });
  res.send( { status: 1, userData: dbUser, chatroomUsers } );
});
